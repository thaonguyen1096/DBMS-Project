...
BEGIN
	Begin tran
		begin try
		IF @MaNguoiDung NOT in (SELECT MaNguoiDung FROM Users with (XLOCK) WHERE MaNguoiDung = @MaNguoiDung and MatKhau = @MatKhau and TinhTrang = '1')
		BEGIN
...
BEGIN
	BEGIN TRAN
		...
		--Kiểm tra người dùng tồn tại:
			if exists (select * from Users where MaNguoiDung = @MaNguoiDung )
...
BEGIN
	BEGIN TRANSACTION
		BEGIN TRY  
		IF NOT EXISTS (SELECT*FROM Users with (XLOCK) WHERE MaNguoiDung = @manguoidung AND MatKhau = @matkhaucu AND TinhTrang='1')
			BEGIN
				SET @kq = '0'
...
begin
	...
	begin try 
	if not exists(select*from DangKyDeCaNhan with (XLOCK) where MSSV = @mssv and MaDe = @made and MaMH = @mamon and TrangThai= 1)
	begin
		set @kq = 0;
...
begin
	...
	begin try 
		if not exists(select*from DangKyDeNhom with (XLOCK) where MaNhom = @manhom and MaDe = @made and MaMH = @mamon and TrangThai= 1)
		begin
			set @kq = 0;
			raiserror(N'Sinh viên chưa đăng ký đề môn này', 16, 1);
			rollback tran huyDangKyDeCaNhan;  
...
AS
	BEGIN TRANSACTION
		begin try
			IF NOT EXISTS (SELECT * FROM De_Mon with (XLOCK)  WHERE MaDe = @maDe AND MaMH = @maMon AND TinhTrang = 1)
			BEGIN
				RAISERROR (N'Đề không tồn tại trong môn học này!!!', 16, 1)
				SET @kq = 0
				ROLLBACK TRANSACTION
...
BEGIN
	BEGIN TRAN
		BEGIN TRY
			...
			set @tam = (select count(*) from Mon with (XLOCK))
			set @tam = @tam + 1;
			if (@tam < 10)
...
begin
	...
	--set tran isolation level repeatable read
		begin try
			declare @dem int
			set @dem = (select count(*) from De with (XLOCK))
			if @kq != 0
				waitfor delay '00:00:05'
			set @dem = @dem + 1;
			if (@dem < 10)
...
begin tran
	begin try
		...
		set @Count = (select count(*) from Nhom with (XLOCK)) 
		if @kq != 0
			waitfor delay '00:00:05'
		set @Count = @Count + 1
		if (@Count < 10) 
...
begin transaction
	begin try
		...
		if (@LoaiNguoiDung = '0')
		begin
			set @tam = (select count(*) from Users with (XLOCK) where LoaiNguoiDung = '0')
			set @str = 'AD'
		end
		else 
			if (@LoaiNguoiDung = '1')
			begin
				set @tam = (select count(*) from Users with (XLOCK) where LoaiNguoiDung = '1')
				set @str = 'GV'
			end
			else
			begin
				set @tam = (select count(*) from Users with (XLOCK) where LoaiNguoiDung = @LoaiNguoiDung and Khoa = @Khoa)
...
begin
	begin transaction
		begin try
			...
			SET @sl = (Select SLThanhVien from Nhom with (XLOCK, ROWLOCK) where Mon = @MaMH and MaNhom = @MaNhom)
			if @kq != 0
				waitfor delay '00:00:05'
...
BEGIN
	BEGIN TRANSACTION
		BEGIN TRY
			IF NOT EXISTS (SELECT*FROM De with (XLOCk, ROWLOCK) WHERE MaDe = @made)
				BEGIN
...
AS
	BEGIN TRANSACTION
		BEGIN TRY
			IF NOT EXISTS (SELECT MaMH FROM Mon with (XLOCK, ROWLOCK) WHERE MaMH = @MaMH AND TinhTrang = 1)
				BEGIN
...
AS
	BEGIN TRANSACTION
		BEGIN TRY
			...
			UNION
			select dkdn.MaDe, dm.MaMH, Mota, l.TenLoaiDA, dm.LoaiDe, u.HoTen, dkdn.MaNhom as Nhom
				from DangKyDeNhom dkdn, De_Mon dm, QuanLiNhom qln, Users u, De d, LoaiDoAn l
